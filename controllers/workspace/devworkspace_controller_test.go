...
var _ = Describe("DevWorkspace Controller", func() {
	...
	Context("Init container imagePullPolicy", func() {
		...
		It("Defaults restore imagePullPolicy to IfNotPresent", func() {
			config.SetGlobalConfigForTesting(&controllerv1alpha1.OperatorConfiguration{
				Workspace: &controllerv1alpha1.WorkspaceConfig{
					BackupCronJob: &controllerv1alpha1.BackupCronJobConfig{
						Enable: ptr.To[bool](true),
			...
			for i := range deploy.Spec.Template.Spec.InitContainers {
				if deploy.Spec.Template.Spec.InitContainers[i].Name == restore.WorkspaceRestoreContainerName {
					restoreContainer = &deploy.Spec.Template.Spec.InitContainers[i]
				}
			}
			Expect(restoreContainer).NotTo(BeNil(), "workspace-restore init container should be present")
			Expect(restoreContainer.ImagePullPolicy).To(Equal(corev1.PullIfNotPresent), "workspace-restore should default to IfNotPresent")
		})
